...
DEFAULT_FIXTURE = ROOT / "fixtures" / "continuation-regression-task-371" / "cases.json"
DEFAULT_OUTPUT = ROOT / "outputs" / "task-371-continuation-regression-2026-05-07"
...
def evaluate_case(case: dict[str, Any], payload: dict[str, Any]) -> dict[str, Any]:
    ...
    top_path = path_of(top)
    top_authority = authority_of(top)
    top_match_reason = match_reason_of(top)
    ...
    if "request_class" in expected:
        ...
        add_check("request_class", actual == expected["request_class"], expected["request_class"], actual)

    if "top_path_contains" in expected:
        needle = expected["top_path_contains"]
        add_check("top_path_contains", needle in top_path, needle, top_path)

    if "top_path_contains_any" in expected:
        needles = list(expected["top_path_contains_any"])
        add_check("top_path_contains_any", any(needle in top_path for needle in needles), needles, top_path)

    if "top_authority" in expected:
        add_check("top_authority", top_authority == expected["top_authority"], expected["top_authority"], top_authority)

    for needle in expected.get("top_match_reason_contains", []):
        add_check(f"top_match_reason_contains:{needle}", needle in top_match_reason, needle, top_match_reason)

    for needle in expected.get("top_match_reason_not_contains", []):
        add_check(f"top_match_reason_not_contains:{needle}", needle not in top_match_reason, f"not {needle}", top_match_reason)
    ...
    if "top5_max_duplicate_per_path" in expected:
        actual_top5 = [path_of(item) for item in items[:5]]
        max_duplicate = max((actual_top5.count(path) for path in set(actual_top5)), default=0)
        add_check("top5_max_duplicate_per_path", max_duplicate <= int(expected["top5_max_duplicate_per_path"]), expected["top5_max_duplicate_per_path"], max_duplicate)
    ...
    return {
        ...
        "top_item": {
            "workspace_path": top_path,
            "authority": top_authority,
            "match_reason": top_match_reason,
        },
        "top5_paths": [path_of(item) for item in items[:5]],
        "known_limitation": expected.get("known_limitation"),
    }


def main() -> int:
    parser = argparse.ArgumentParser(description="Verify continuation retrieval regression fixtures for task #371")
    ...
    report: dict[str, Any] = {
        ...
        "contract_path": fixture.get("contract_path"),
        "cases": [],
        "summary": {},
    }

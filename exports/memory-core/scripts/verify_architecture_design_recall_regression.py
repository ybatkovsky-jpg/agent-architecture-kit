...
DEFAULT_FIXTURE = ROOT / "fixtures" / "architecture-design-recall-regression-2026-05-12" / "cases.json"
DEFAULT_OUTPUT = ROOT / "outputs" / "architecture-design-recall-regression-2026-05-12"
...
def evaluate_case(case: dict[str, Any], payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {"pass": ok, "checks": checks, "request_classification": classification, "top_item": {"workspace_path": top_path, "authority": top_authority}, "top5_paths": [path_of(item) for item in items[:5]]}


def main() -> int:
    parser = argparse.ArgumentParser(description="Verify architecture/design recall regression fixtures")

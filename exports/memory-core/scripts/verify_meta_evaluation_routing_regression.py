...
DEFAULT_FIXTURE = ROOT / "fixtures" / "meta-evaluation-routing-regression-2026-05-12" / "cases.json"
DEFAULT_OUTPUT = ROOT / "outputs" / "meta-evaluation-routing-regression-2026-05-12"
...
def evaluate_case(case: dict[str, Any], payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if "top_path_contains" in expected:
        add_check("top_path_contains", expected["top_path_contains"] in top_path, expected["top_path_contains"], top_path)
    ...
    return {"pass": ok, "checks": checks, "request_classification": classification, "top_item": {"workspace_path": top_path, "authority": top_authority, "match_reason": top_match_reason}, "top5_paths": [path_of(item) for item in items[:5]]}


def main() -> int:
    parser = argparse.ArgumentParser(description="Verify meta/evaluation routing regression fixtures")

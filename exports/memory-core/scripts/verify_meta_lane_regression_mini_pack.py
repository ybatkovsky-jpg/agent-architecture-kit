...
DEFAULT_FIXTURE = ROOT / "fixtures" / "meta-lane-regression-mini-pack-2026-05-07" / "cases.json"
DEFAULT_OUTPUT = ROOT / "outputs" / "meta-lane-regression-mini-pack-2026-05-07"
...
def evaluate_case(case: dict[str, Any], payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if "meta_subfamily" in expected:
        actual = classification.get("meta_subfamily")
        add_check("meta_subfamily", actual == expected["meta_subfamily"], expected["meta_subfamily"], actual)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Verify compact meta-lane regression mini-pack")

...
This document defines the minimum continuation contract for the Frame pattern:
...
handoff:
  ...
  return_target: parent the earlier implementation work note and parent session summary
...
This document is a useful bounded reference for #394 because it:
...
This artifact also appears **public-safe and reusable enough for later promotion** into `product-repos/agent-architecture-kit` in product-facing form.
